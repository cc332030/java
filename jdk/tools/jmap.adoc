...
PID=14424
PID=$(jps -l | grep "${APP}" | cut -d ' ' -f 1)

# 查看堆栈占用空间，新生代，老年代
jmap -heap ${PID}  > jmap-heap-${PID}.txt
jmap -heap $(jps -l | grep "${APP}" | cut -d ' ' -f 1)

# 查看所有对象及数量，包括活跃以及非活跃的
jmap -histo ${PID} > jmap-histo-${PID}.txt
jmap -histo $(jps -l | grep "${APP}" | cut -d ' ' -f 1) | less

# 查看活跃对象及数量
jmap -histo:live ${PID} > jmap-histo-live-${PID}.txt
jmap -histo:live ${PID} | less

# 导出内存信息
jmap -dump:format=b,file=${PID}-1.hprof ${PID}
# 导出内存信息，强制
jmap -F -dump:format=b,file=${PID}-1.hprof ${PID}
